...
| **압축 비율 (모델 차원)** | **1.5 : 1**  | 12 피처 → 8 잠재 벡터 |
| **압축률 (전송 페이로드)** | **62.5%**    | 32B → 20B (오버헤드 포함) |
...
- **입력 피처**: 12 개
- **잠재 벡터**: 8 개
- **Compression ratio (모델 차원)**: **1.5 : 1**
- **압축률 (전송 페이로드 기준, 오버헤드 포함)**: **62.5%** (32B → 20B)
...
> - gyro(gx/gy/gz)는 정지 상태에서 측정해 원본 값이 0이었고, altitude는 변화가 적은 GPS 고도값을 사용했습니다. 두 항목의 오차가 큰 것은 데이터 특성에 따른 것으로, 움직임이 있는 시계열 데이터 구성이 압축 학습에 더 적합했을 것으로 분석됩니다.
